...
const BENCHMARKS: BenchmarkRow[] = [
  { metric: 'Daily Active Users', vertexchain: 4_200, industry: 3_500, unit: '' },
  { metric: 'Engagement Rate',    vertexchain: 38,    industry: 45,    unit: '%' },
  { metric: 'Retention Rate',     vertexchain: 62,    industry: 55,    unit: '%' },
  { metric: 'Avg Session Length', vertexchain: 4.2,   industry: 5.1,   unit: 'min' },
];

function pctDiff(vertexchain: number, industry: number): number {
  return Number((((vertexchain - industry) / industry) * 100).toFixed(1));
}
...
export default function BenchmarkComparison() {
  ...
  const vertexchainNorm = BENCHMARKS.map((r) => Number(((r.vertexchain / r.industry) * 100).toFixed(1)));
  ...
  const data = {
    ...
    datasets: [
      {
        label: 'VertexChain',
        data: vertexchainNorm,
        backgroundColor: vertexchainNorm.map((v) => (v >= 100 ? 'rgba(34,197,94,0.8)' : 'rgba(239,68,68,0.8)')),
        borderRadius: 6,
        barPercentage: 0.45,
      },
      {
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          <thead>
            <tr style={{ borderBottom: '2px solid #e5e7eb' }}>
              {['Metric', 'VertexChain', 'Industry Avg', '% Diff'].map((h) => (
          ...
          <tbody>
            {BENCHMARKS.map((row) => {
              const diff = pctDiff(row.vertexchain, row.industry);
              const above = diff >= 0;
              return (
                <tr key={row.metric} style={{ borderBottom: '1px solid #f1f5f9' }}>
                  <td style={{ padding: '10px 12px', fontWeight: 600 }}>{row.metric}</td>
                  <td style={{ padding: '10px 12px' }}>{row.vertexchain.toLocaleString()}{row.unit && ` ${row.unit}`}</td>
